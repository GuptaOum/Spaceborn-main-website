...
function CapabilityMatrix({ measured }: { measured: number }) {
  return <div className="capability-matrix" aria-label={`${measured} of 66 telemetry fields measured`}>
    {Array.from({ length: 66 }, (_, index) => <i className={index < measured ? 'measured' : ''} key={index} title={`Telemetry field ${String(index + 1).padStart(2, '0')}`} />)}
  </div>;
}

function TelemetryInstrument() {
  return <div className="telemetry-instrument">
    <div className="instrument-gauge"><div className="gauge-arc" /><strong>78<span>%</span></strong><small>BATTERY</small></div>
    <div className="instrument-compass"><span>N</span><i /><b>WIND 04.2 m/s</b></div>
    <div className="instrument-horizon"><i /><b>GPS LOCK</b><small>11 SATELLITES</small></div>
    <div className="instrument-signal"><small>LATENCY</small><strong>018 ms</strong><span /></div>
  </div>;
}
...
export default function DerykPage() {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.deryk-section, .deryk-trust, .deryk-cta').forEach((element) => revealObserver.observe(element));
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      page.style.setProperty('--parallax-x', `${x * 8}px`);
      page.style.setProperty('--parallax-y', `${y * 8}px`);
    };
    window.addEventListener('pointermove', onPointerMove, { passive: true });
    return () => window.removeEventListener('pointermove', onPointerMove);
  }, []);

  return <div className="deryk-page" id="top" ref={pageRef}>
    ...
    <main>
      ...
      <section className="deryk-section" id="studios"><SectionHeading eyebrow="02 / Platform studios" title="One control plane. Every flight signal." copy="A focused workspace for the whole mission loop, from a first sentence to a defensible flight record." /><div className="deryk-studios">{[['Console Studio', 'Fly missions using natural language', Terminal], ['Connector Studio', 'Connect drones and capability masks', Network], ['Telemetry Studio', 'Live charts and sensor visibility', BarChart3], ['Flight Records', 'Replay missions and audit trails', Database]].map(([title, copy, Icon], index) => <article data-index={`0${index + 1}`} key={title as string}><Icon size={22} /><small>DERYK STUDIO / 0{index + 1}</small><h3>{title as string}</h3><p>{copy as string}</p><div className={`deryk-preview preview-${index}`}><span>{title === 'Console Studio' ? '> fly a perimeter around sector 04' : title === 'Connector Studio' ? 'CERTAINTY 45 / 66' : title === 'Telemetry Studio' ? 'ALTITUDE 124.6 m' : 'OPIP-2048 / GATE VERIFIED'}</span>{index === 0 && <em>planning <b>▋</b></em>}{index === 1 && <div className="preview-devices"><b /> CERTAINTY <strong>ONLINE</strong><b /> LIDAR-SIM <strong>MASKED</strong></div>}{index === 2 && <div className="preview-wave" />}{index === 3 && <div className="preview-events"><b>GATE VERIFIED</b><b>TELEMETRY EXPORTED</b></div>}</div><a href="#observability">Open studio <ArrowRight size={14} /></a></article>)}</div></section>
      ...
      <section className="deryk-section deryk-telemetry-band"><SectionHeading eyebrow="05.5 / Live signal layer" title="The machine only knows what it measures." copy="Telemetry stays observable, honest, and outside the safety path." /><TelemetryInstrument /><div className="telemetry-readout"><span>CONNECTOR / CERTAINTY</span><b>ONLINE</b><span>LAST FRAME / 018 ms AGO</span><b>STABLE</b><span>ANALYTICS BUS / BEST EFFORT</span><b>ISOLATED</b></div></section>

      <section className="deryk-section deryk-capability-grid"><div><SectionHeading eyebrow="05.75 / Capability matrix" title="66 fields. No invented certainty." copy="Every cell represents a real signal contract. Bright means measured. Dim means not measured." /><div className="matrix-legend"><span><i className="measured" /> MEASURED</span><span><i /> NOT MEASURED</span></div></div><div><div className="matrix-card"><header><b>Certainty</b><strong>45 / 66</strong></header><CapabilityMatrix measured={45} /></div><div className="matrix-card muted"><header><b>Lidar Simulator</b><strong>11 / 66</strong></header><CapabilityMatrix measured={11} /></div></div></section>
